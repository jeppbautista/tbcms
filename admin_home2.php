...
//----------------------------------------------------------------------------------------SIGNUP_FORM START
	...
	if(isset($_POST['temporary_value'])) {
		...
		if($temvalue=='') {
			...
			$query="select * from xtbl_admin_transaction2 WHERE Ctr='$temvalue'";
			$rs=mysql_query($query);
			$row=mysql_fetch_assoc($rs);
			...
			$query="update xtbl_admin_transaction2 SET Status='SUCCESS' 
				WHERE Ctr='$temvalue'";
			$rs=@mysql_query($query);
		...
		if($temvalue=='') {
			...
			if($row['Remarks']=='ACTIVATION') {
				$query="update xtbl_account_info SET Account_Status='INACTIVE' 
					WHERE Ctr='$temp_ctr'";
				$rs=@mysql_query($query);
				$query="delete from xtbl_admin_transaction2 WHERE Ctr='$temvalue'";
				$rs=@mysql_query($query);
			}

			$query="update xtbl_admin_transaction2 SET Status='DENIED' 
				WHERE Ctr='$temvalue'";
			$rs=@mysql_query($query);
			echo '<script>window.location.assign("https://tbcmerchantservices.com/admin_home/");</script>';
	...
	$class->html_start('');
		...
		?>
			...
			<div class="container">
				...
				$query="select * from xtbl_admin_transaction2 WHERE Status='WAITING' Order by Datetime DESC";
				$rs=mysql_query($query);
				...
				?>
					<tr>
						...
						<td width="40%"><?php echo $row['Transaction'];?></td>
				...
				<?php
					...
					$query="select * from xtbl_admin_transaction2 WHERE Status<>'WAITING' Order by Datetime DESC LIMIT 500";
					$rs=mysql_query($query);
					while($row=mysql_fetch_assoc($rs)) {
				?>
					<tr>
